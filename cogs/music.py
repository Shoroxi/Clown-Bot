import discord,asyncio,time,datetime,random,youtube_dl,string,os,functools
...
class MusicPlayer(commands.Cog):
    ...
    async def play(self,msg,*,song):
        """Текущие рабочие сервисы: Youtube
        Пример: ~play https://www.youtube.com/watch?v=jpUpX--8ucI"""
    ...
    @command(aliases=['song-list','q','current-songs'])
    async def qqueue(self,msg):
    ...
    async def join(self, msg, *, channel: discord.VoiceChannel=None):
        """Чтобы бот зашел в голосовой чат """
    ...
    async def leave(self,msg):
        """Чтобы бот вышел в голосовой чат """
        if msg.voice_client != None:
            if msg.voice_client != None and msg.voice_client.is_playing() == False and msg.voice_client.is_paused() == False:
                await msg.voice_client.disconnect()
